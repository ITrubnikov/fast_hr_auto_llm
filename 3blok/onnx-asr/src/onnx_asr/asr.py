...
class Asr(ABC):
    def __init__(self, model_files: dict[str, Path], session_options):
...
class _AsrWithDecoding(Asr):
    ...
    def __init__(self, model_files: dict[str, Path], session_options):
        super().__init__(model_files, session_options)
    ...
    def recognize_batch(
        ...
        encoder_out, encoder_out_lens = self._encode(waveforms, waveforms_len)
        return (
